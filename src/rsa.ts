...
async function main() {
  ...
  const updateKeys = () => {
    try {
      ...
      parsedPubKeys = cryptos.parsePublicKeyPem(pubPem);
    } catch (e) { parsedKeysa = parsedPubKeys = null; }
  };
  ...
  btns.sign.textContent = "署名する"; btns.verify.textContent = "検証する";
  btns.enc.textContent = "暗号化する"; btns.dec.textContent = "復号化する";
  btns.copy.textContent = "結果をコピー"; btns.copy.style.color = "blue";
  btns.clear.textContent = "入力を削除"; btns.clear.style.color = "red";
  ...
  btns.copy.onclick = async () => {
    ...
    if (text) {
      ...
      if (isinmumode) processMemeEffect("copy", true); playSpecialAudio(text);
  ...
  if (urlParams.get("roop") === "true") {
    alert("不正な操作が検出されました。");
    for (let i = 0; i < 1000; i++) { console.log("System loop..."); }
