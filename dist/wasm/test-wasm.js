...
class WasmPerformanceTest {
     ...
     */
    modExpMontgomeryPure(base, exp, modulus) {
        const bitLength = (n) => {
            if (n === 0n)
                return 0;
    ...
    testSummary() {
        ...
        for (const bits of testSizes) {
            ...
            const wasmStart = performance.now();
            for (let i = 0; i < 10; i++)
                this.bigint.modExpMontgomery(base, exp, mod);
            const wasmTime = performance.now() - wasmStart;
            const jsStart = performance.now();
            for (let i = 0; i < 10; i++)
                this.modExpMontgomeryPure(base, exp, mod);
            const jsTime = performance.now() - jsStart;
...
if (typeof window !== 'undefined') {
    window.WasmPerformanceTest = WasmPerformanceTest;
