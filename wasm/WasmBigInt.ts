...
export interface WasmExports extends WebAssembly.Exports {
  ...
  add: (a: number, b: number, result: number, limbs: number) => number;
  sub: (a: number, b: number, result: number, limbs: number) => number;
  mul: (a: number, b: number, result: number, aLimbs: number, bLimbs: number) => void;
  div: (dividend: number, divisor: number, quotient: number, remainder: number, limbs: number) => void;
  cmp: (a: number, b: number, limbs: number) => number;
  mod: (a: number, n: number, result: number, aLimbs: number, nLimbs: number) => void;
  modExp: (base: number, exp: number, mod: number, result: number, limbs: number) => void;
  modExpMontgomery: (base: number, exp: number, mod: number, result: number, limbs: number) => void;
}
...
export class WasmBigInt {
   ...
   * @param limbs limb数
   */
  writeBigInt(ptr: number, value: bigint, limbs: number): void {
    this.checkInit();
   ...
   * @param limbs limb数
   */
  readBigInt(ptr: number, limbs: number): bigint {
    this.checkInit();
    const view = new BigUint64Array(this.memory!.buffer);
    const offset = ptr / 8;
    let result = 0n;
   ...
   */
  calcMaxLimbs(...numbers: bigint[]): number {
   ...
   * 除算: a / b
   */
  div(a: bigint, b: bigint): DivResult {
    this.checkInit();
   ...
   */
  modExp(base: bigint, exp: bigint, modulus: bigint): ModExpResult {
    this.checkInit();
   ...
   */
  modExpMontgomery(base: bigint, exp: bigint, modulus: bigint): ModExpResult {
    this.checkInit();
   ...
   * RSA暗号化: m^e mod n
   */
  rsaEncrypt(message: bigint, e: bigint, n: bigint): bigint {
    if (message >= n) {
   ...
   * RSA復号: c^d mod n
   */
  rsaDecrypt(ciphertext: bigint, d: bigint, n: bigint): bigint {
    if (ciphertext >= n) {
   ...
   * RSA完全テスト（暗号化→復号→検証）
   */
  testRSA(message: bigint, e: bigint, d: bigint, n: bigint): {
    success: boolean;
    ciphertext: bigint;
    decrypted: bigint;
    encryptTime: number;
   ...
   * RSA鍵ペア生成用のヘルパー関数
   */
  static generateRSAKeys(p: bigint, q: bigint, e: bigint = 65537n): {
    n: bigint;
    e: bigint;
    d: bigint;
    phi: bigint;
